...
class DataBase:
    ...
    WATER_MARK_TYPE = ("Horzontal4_3", "Horzontal16_9", "Square1_1",
                       "Vertical3_4", "Vertical9_16", "Folder")
    WATER_MARK_SIZE = ((1280, 960), (1280, 720), (1280, 1280), (960, 1280),
                       (720, 1280), (0, 0))
    ...
    WATER_MARK_BACKUP = os.path.join(
        os.getenv("TEMP"), "Auto_Water_Mark_Backup")

    WATER_MARK_RATIO = ((1280 / 960, (1280 - WATER_MARK_TOLERANCE) / 960,
                         1280 / (960 - WATER_MARK_TOLERANCE)),
                        (1280 / 720, (1280 - WATER_MARK_TOLERANCE) / 720,
                         1280 / (720 - WATER_MARK_TOLERANCE)),
                        (1280 / 1280, (1280 - WATER_MARK_TOLERANCE) / 1280,
                         1280 / (1280 - WATER_MARK_TOLERANCE)),
                        (960 / 1280, (960 - WATER_MARK_TOLERANCE) / 1280,
                         960 / (1280 - WATER_MARK_TOLERANCE)),
                        (720 / 1280, (720 - WATER_MARK_TOLERANCE) / 1280,
                         720 / (1280 - WATER_MARK_TOLERANCE)), (0, 0, 0))

    auto_shade_collection = collections.namedtuple("auto_shade_group",
                                                   "day night brighter")
    ...
    AUTO_SHADE_HISTOGRAM_DIFFERENCE_THRESHOLD = auto_shade_collection(
        0.115, 0.115, 0.3)
    ...
    AUTO_SHADE_COMMAND = auto_shade_collection(AUTO_SHADING_HARD_COMMAND,
                                               AUTO_SHADING_DARK_COMMAND,
...
class MainUI(qw.QDialog):
    ...
    def __init__(self):
        ...
        self.setWindowFlags(self.windowFlags() | qc.Qt.WindowMinimizeButtonHint
        ...
        self.text_raw_render_output.setPlaceholderText(
            "The Output Path of Rendering")
        ...
        self.text_final_output.setPlaceholderText(
            "Path to store the water-marked and auto-shaded image")
        ...
        self.slider.valueChanged.connect(
            lambda: self.file_list.slider_change_icon_size(self.slider.value()))
    ...
    def run_button_click(self):
        ...
        if self.server_running is False:
            ...
            raw_render = self.text_raw_render_output.toPlainText().replace(
                DataBase.URL_START, "")
            final_path = self.text_final_output.toPlainText().replace(
                DataBase.URL_START, "")

            if not raw_render or not os.path.isdir(raw_render):
                qw.QMessageBox.information(
                    qw.QMessageBox(), "Information",
                    "Please specify the path of render output")
                return
    ...
    def render_comp_check(self):
        ...
        if self.check_server_mode.isChecked():
            self.check_duplicate_original.setEnabled(
                not self.check_duplicate_original.isEnabled())

        self.text_raw_render_output.setEnabled(
            not self.text_raw_render_output.isEnabled())
        self.text_final_output.setEnabled(
            not self.text_final_output.isEnabled())
        self.check_server_mode.setEnabled(
            not self.check_server_mode.isEnabled())
        self.button_run.setEnabled(not self.button_run.isEnabled())

    def server_mode_check(self):

        self.check_duplicate_original.setEnabled(
            self.check_server_mode.isChecked())

        if self.check_server_mode.isChecked():
            self.text_final_output.setPlaceholderText(
                "Alternative Path to store the water_marked image")

        else:
            self.text_final_output.setPlaceholderText(
                "Folder Path for Render Output")
...
#####
        ...
        bright_ratio = bright_luminance_average / (
            bright_luminance_average + dark_luminance_average)

        if bright_ratio > DataBase.AUTO_SHADE_HISTOGRAM_DIFFERENCE_THRESHOLD.brighter:
            shade[DataBase.AUTO_SHADE_TYPE.brighter].append(
...
#####
        ...
        if (x, y) not in DataBase.WATER_MARK_SIZE:
            ...
            for j in range(len(DataBase.WATER_MARK_SIZE)):
                ...
                func((x, y), DataBase.WATER_MARK_RATIO[j],
                     output[DataBase.WATER_MARK_TYPE[j]], file)

        image.close()

    @staticmethod
    def image_size_check_ratio(input_parm, compare_parm, output_item,
                               output_append_item):
    ...
    @staticmethod
    def image_size_check_pixel(tolerance, input_parm, compare_parm, output_item,
                               output_append_item):
    ...
    def go_button_clicked(self):
        ...
        if not self.check_water_mark.isChecked(
        ) and not self.check_auto_curve.isChecked():
            qw.QMessageBox.information(
                qw.QMessageBox(), "Information",
        ...
        for i in self.file_list.contain:
            ...
            if os.path.isfile(i):
                self.image_size_dict_distribution(
                    i, self.image_size_check_ratio, self.output_dict,
                ...
                if not self.check_recursive.isChecked():
                    ...
                    for j in os.listdir(i):
                        ...
                        if os.path.isfile(file_path):
                            ...
                            self.image_size_dict_distribution(
                                file_path, self.image_size_check_ratio,
    ...
    def process_command(self, ratio_key, value_element):
        ...
        if self.check_auto_curve.isChecked():
            ...
            if os.path.basename(file_path) in self.auto_shade_dict[
                    DataBase.AUTO_SHADE_TYPE.brighter]:
                auto_shade_command.append(
                    os.path.join(DataBase.AUTO_SHADING_LOCATION,
                                 DataBase.AUTO_SHADE_COMMAND.brighter))

            elif os.path.basename(file_path) in self.auto_shade_dict[
                    DataBase.AUTO_SHADE_TYPE.day]:
                auto_shade_command.append(
                    os.path.join(DataBase.AUTO_SHADING_LOCATION,
...
class NameList(qw.QListWidget):
    ...
    def __init__(self, dialog):
        ...
        self.setIconSize(
            qc.QSize(DataBase.ICON_START_SIZE, DataBase.ICON_START_SIZE))
